...
func (c *ConnnectRessource) refreshTokenGrant(req *refreshTokenTokenRequest, w http.ResponseWriter, r *http.Request) {
	auth, err := c.autService.AuthorizationByCommonToken(r.Context(), string(tokens.RefreshTokenType), req.refreshToken)
	if err != nil {
	...
	if app.HasSecret() && req.clientID == "" {
		render.Respond(w, r, createStdError(stdInvalidClient, http.StatusBadRequest, "this client requires client_id"))
		return
	}
	...
	err = c.rotator.RotateCommonToken(r.Context(), tokens.RefreshTokenType, req.refreshToken, app.ClientID())
	if err != nil {
	...
	user, err := c.userSignIn.SignInByIDFromToken(r.Context(), auth.UserID(), string(tokens.RefreshTokenType))
	if err != nil {
